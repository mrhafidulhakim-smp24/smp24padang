...
import { wasteDocumentation, wasteNews, wasteVideos } from "@/lib/db/schema";
import {
...
export async function createWasteNews(prevState: unknown, formData: FormData) {
  ...
  try {
    ...
    if (imageUrl) {
      try {
        await deleteImageFromSupabase(imageUrl, "banksampah/news");
      } catch {
        /* ignore */
      }
    }
    ...
    return {
      success: false,
      message: "Gagal menambahkan berita karena error tidak diketahui.",
    };
  }
}

export async function updateWasteNews(
  id: number,
  prevState: unknown,
  formData: FormData,
) {
  ...
  const existing = await db.query.wasteNews.findFirst({
  ...
  try {
    ...
    if (newUrl) {
      try {
        await deleteImageFromSupabase(newUrl, "banksampah/news");
      } catch {
        /* ignore */
      }
    }
    ...
    return {
      success: false,
      message: "Gagal memperbarui berita karena error tidak diketahui.",
    };
  }
}
...
export async function createWasteDocumentation(
  ...
  try {
    ...
    if (imageUrl) {
      try {
        await deleteImageFromSupabase(imageUrl, "banksampah/documentation");
      } catch {
        /* ignore */
      }
    }
...
export async function updateWasteDocumentation(
  id: number,
  prevState: unknown,
  formData: FormData,
) {
  ...
  let newUrl: string | null = null;
  const existing = await db.query.wasteDocumentation.findFirst({
  ...
  try {
    if (imageFile && imageFile.size > 0) {
      newUrl = await uploadImageToSupabase(
        imageFile,
        "banksampah/documentation",
      );
    ...
    if (newUrl) {
      try {
        await deleteImageFromSupabase(newUrl, "banksampah/documentation");
      } catch {
        /* ignore */
      }
    }
    if (error instanceof Error) {
      return {
        success: false,
        message: `Gagal memperbarui dokumentasi: ${error.message}`,
      };
    }
...
export async function createWasteVideo(prevState: unknown, formData: FormData) {
  ...
  try {
    ...
    return {
      success: false,
      message: "Gagal menambahkan video karena error tidak diketahui.",
    };
  }
}

export async function updateWasteVideo(
  id: number,
  prevState: unknown,
  formData: FormData,
) {
  ...
  try {
    ...
    if (error instanceof Error) {
      return {
        success: false,
        message: `Gagal memperbarui video: ${error.message}`,
      };
    }
    return {
      success: false,
      message: "Gagal memperbarui video karena error tidak diketahui.",
    };
  }
}
