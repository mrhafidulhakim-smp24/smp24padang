...
async function ensureKelasSeeded() {
  const existingKelas = await db.select({ id: kelas.id }).from(kelas).limit(1);
...
export async function createJenisSampah(data: {
  ...
  if (
    !data.namaSampah.trim() ||
    !Number.isFinite(data.hargaPerKg) ||
    data.hargaPerKg < 0
  ) {
...
export async function deleteJenisSampah(id: number) {
  try {
    const [setoranKelas, setoranGuruData, setoranMasyarakatData] =
      await Promise.all([
        db
          .select({ id: sampahKelas.id })
          .from(sampahKelas)
          .where(eq(sampahKelas.jenisSampahId, id))
          .limit(1),
        db
          .select({ id: setoranGuru.id })
          .from(setoranGuru)
          .where(eq(setoranGuru.jenisSampahId, id))
          .limit(1),
        db
          .select({ id: setoranMasyarakat.id })
          .from(setoranMasyarakat)
          .where(eq(setoranMasyarakat.jenisSampahId, id))
          .limit(1),
      ]);
    if (
      setoranKelas.length ||
      setoranGuruData.length ||
      setoranMasyarakatData.length
    ) {
      return {
        error:
          "Jenis sampah sudah dipakai pada setoran dan tidak boleh dihapus.",
      };
    }
...
export async function updateSampahKelas(
  ...
  try {
    ...
    await db.update(sampahKelas).set(updateData).where(eq(sampahKelas.id, id));
    revalidatePath("/admin/sispendik");
    revalidatePath("/sispendik");
    return { success: true };
  } catch (error) {
...
export async function getLaporanPerolehanTahunan(year: number) {
  ...
  try {
    ...
    return {
      error:
        "Gagal membuat laporan perolehan. Pastikan migrasi SISPENDIG sudah dijalankan.",
    };
  }
}
...
export async function getPerkembanganSampahKelas(year: number) {
  ...
  try {
    ...
    const byClassMonth = new Map(
      totals.map((item) => [
        `${item.kelasId}-${Number(item.month)}`,
