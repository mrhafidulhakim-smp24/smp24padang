...
import { Pencil, Plus, Printer, RotateCcw, Save, Trash2 } from "lucide-react";
...
export function TabSetoranKelas({ kelas, jenisSampah }: TabSetoranKelasProps) {
  ...
  const [newJenisKategori, setNewJenisKategori] = useState<
    "organik" | "anorganik"
  >("anorganik");
  ...
  const [selectedEntry, setSelectedEntry] = useState<SetoranEntry | null>(null);
  ...
  const fetchRecap = useCallback(async () => {
    ...
    if (totals.data && Array.isArray(totals.data) && totals.data.length > 0) {
    ...
    <div className="space-y-6">
                  ...
                  <TableBody>
                    ...
                    {filteredRows.map((row: any, idx: number) => (
                      <TableRow key={`${row.kelasId}-${idx}`}>
                        ...
                        <TableCell>
                          Rp{" "}
                          {Number(row.totalValue || 0).toLocaleString("id-ID", {
                            minimumFractionDigits: 0,
                  ...
                  <tfoot>
                    <TableRow>
                      ...
                      <TableCell className="font-semibold">
                        {filteredSumKg.toLocaleString("id-ID", {
                          maximumFractionDigits: 2,
                        })}
                      </TableCell>
                      <TableCell className="font-semibold">
                        Rp {filteredSumValue.toLocaleString("id-ID")}
                      </TableCell>
          ...
          <CardContent>
            <div className="space-y-2">
              {localJenis.map((j) => (
                ...
                >
                  <div>
                    <div className="font-medium">{j.namaSampah}</div>
                    <div className="text-sm text-muted-foreground">
                      Rp {Number(j.hargaPerKg).toLocaleString("id-ID")}
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setNewJenisPrice(String(j.hargaPerKg));
                        setNewJenisKategori(
                          j.kategori === "organik" ? "organik" : "anorganik",
                        );
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingJenis ? "Edit Jenis Sampah" : "Tambah Jenis Sampah"}
            </DialogTitle>
            <DialogDescription>
              {editingJenis
          ...
          <form onSubmit={handleCreateOrUpdateJenis} className="space-y-4">
            <div>
            ...
            <div>
              ...
              <Input
                value={newJenisPrice}
                onChange={(e) => setNewJenisPrice(e.target.value)}
                type="number"
                step="0.01"
              />
            </div>
      ...
      >
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              Kelola Setoran{" "}
              {manageClass ? `${manageClass.tingkat}${manageClass.huruf}` : ""}{" "}
          ...
          >
            <div className="sm:col-span-2">
              ...
              <Select value={addJenisId} onValueChange={setAddJenisId}>
                <SelectTrigger>
                  <SelectValue placeholder="Pilih jenis" />
                </SelectTrigger>
                <SelectContent>
                  {localJenis.map((j) => (
                    <SelectItem value={String(j.id)} key={j.id}>
                      {j.namaSampah}
                    </SelectItem>
                  ))}
                </SelectContent>
                    ...
                    <TableRow key={e.id}>
                      ...
                      <TableCell>
                        {isEditing ? (
                          <Input
                            value={editJumlah}
                            onChange={(ev) => setEditJumlah(ev.target.value)}
                            type="number"
                            step="0.01"
                          />
                        ) : (
                      ...
                      <TableCell>Rp {harga.toLocaleString("id-ID")}</TableCell>
                      <TableCell>
            ...
            <AlertDialogAction
              onClick={async () => {
                ...
                if ((res as any).success) {
                  toast({
                    title: "Sukses",
                    description: "Setoran bulan ini direset",
                  });
                  fetchRecap();
                } else {
                  toast({
                    title: "Error",
                    description: (res as any).error || "Gagal reset",
                    variant: "destructive",
                  });
                }
