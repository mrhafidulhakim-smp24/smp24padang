...
export async function getAccreditations() {
  try {
    const result = await db.select().from(accreditations).orderBy(desc(accreditations.createdAt));
    return result;
  } catch (error) {
...
export async function createAccreditation(data: { title: string; description: string; link: string; }) {
  try {
...
export async function updateAccreditation(id: string, data: { title: string; description: string; link: string; }) {
  try {
    await db
