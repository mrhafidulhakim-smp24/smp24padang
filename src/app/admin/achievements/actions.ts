...
export async function updateAchievement(id: string, currentImageUrl: string, formData: FormData) {
    const validatedFields = AchievementSchema.safeParse({
        title: formData.get('title'),
        student: formData.get('student'),
        description: formData.get('description'),
...
export async function deleteAchievement(id: string, imageUrl: string) {
    try {
