...
export default function SispendikDashboard() {
  ...
  const [printSection, setPrintSection] = useState<
    "perkembangan" | "guru" | "masyarakat" | null
  >(null);
  ...
  const maxClassTotal =
    classTotals.length > 0 ? Math.max(...classTotals.map((c) => c.total)) : 0;
  const maxGuruKg =
    guruRanking.length > 0 ? Math.max(...guruRanking.map((g) => g.totalKg)) : 0;
        ...
        <Card className="bg-green-100 dark:bg-green-900">
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle>
                {topRankerType === "kelas" ? "Kelas Unggulan" : "Guru Unggulan"}
              </CardTitle>
              <Button
                variant="outline"
                size="sm"
              ...
              >
                Lihat {topRankerType === "kelas" ? "Guru" : "Kelas"}
              </Button>
            </div>
                ...
                {isMobileChart ? (
                  <>
                    <XAxis
                      ...
                      domain={[
                        0,
                        maxClassTotal > 0 ? Math.ceil(maxClassTotal * 1.1) : 10,
                      ]}
                      tick={{ fontSize: 12 }}
                    />
                    <YAxis
                ...
                <TableBody>
                  {classProgress.length === 0 ? (
                    ...
                    classProgress.map((row) => (
                      <TableRow key={row.kelas} className="hover:bg-muted/50">
                ...
                {isMobileChart ? (
                  <>
                    <XAxis
                      ...
                      domain={[
                        0,
                        maxGuruKg > 0 ? Math.ceil(maxGuruKg * 1.1) : 10,
                      ]}
                      tick={{ fontSize: 12 }}
                    />
                    <YAxis
