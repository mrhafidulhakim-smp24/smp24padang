...
import {
  getPublicWasteNews,
  getPublicWasteVideos,
} from "@/app/admin/banksampah/actions";
              ...
              <Card className="overflow-hidden flex flex-col h-full transition-all hover:border-primary">
                <CardHeader>
                  <div className="relative w-full h-48">
                    <Image
                      src={
                        item.previewUrl || "https://placehold.co/600x400.png"
                      }
                      alt={item.title}
                      fill
  ...
  return (
    <div className="space-y-6 pt-5 sm:space-y-8 sm:pt-6">
      ...
      <Dialog
        ...
        onOpenChange={(isOpen) => !isOpen && closeVideo()}
      >
        <DialogContent className="max-w-4xl p-0">
          {selectedVideoUrl && (
            <YouTubeEmbed
              url={selectedVideoUrl}
              title="Video Edukasi Bank Sampah"
            />
          )}
        </DialogContent>
      </Dialog>
