...
export function VideoCard({
  ...
  return (
    <div onClick={onClick} className="cursor-pointer group">
      ...
      <h3 className="font-semibold text-base mt-2 truncate">{video.title}</h3>
      {video.description && <p className="text-sm text-muted-foreground mt-1 truncate">{video.description}</p>}
    </div>
  );
}